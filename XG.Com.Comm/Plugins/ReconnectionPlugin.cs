...
namespace XG.Com.Comm
{
    ...
    public sealed partial class ReconnectionPlugin<TClient> : PluginBase, ILoadedConfigPlugin, ITcpDisconnectedPlugin<TClient> where TClient : class, ITcpClient
    {
        ...
        /// <inheritdoc/>
        public Task OnLoadedConfig(object sender, ConfigEventArgs e)
        {
            Task.Run(async () =>
            {
                if (!this.m_polling)
        ...
        /// <inheritdoc/>
        public Task OnTcpDisconnected(TClient client, DisconnectEventArgs e)
        {
            Task.Run(async () =>
            {
                if (e.Manual)
